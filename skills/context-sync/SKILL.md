...
**When to run:**
- Manually via `/context-sync` any time you want a fresh pull.
- Prompted by the session-start hook when `CONTEXT_STALE=true` is set.

**Preconditions:**
- `context/` must exist. If not, run `/context-init` first.
...
- Review open governance tasks → open `tasks/governance.md` and address each
  conflict manually, then re-run `/context-sync` to confirm resolution.
- Surface active work → `/tasks`
- Full context briefing after sync → `/fetch-context`
