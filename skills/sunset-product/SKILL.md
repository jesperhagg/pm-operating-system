...
- `/context-sync` surfaced a stale bet in governance tasks with no recent activity.
...
Use `/log-decision` logic to append a new H2 block to `context/product/decisions.md`:
...
Append a new H3 to the top of `context/market/signals.md` (via `/log-signal` logic):
...
- **Kill** with a clear replacement idea → `/evaluate-opportunity`.
- **Park** with a revisit trigger → `/pm-digest` or `/market-scan` should surface it when conditions change.
- **Graduate** → `/break-down` to reshape the absorbing product's backlog.
- Multiple bets sunset in a row → `/context-sync` to ensure the context layer is consistent.
