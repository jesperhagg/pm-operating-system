...
│   ├── skills/           # 33 skills, grouped by category; see skills/README.md
...
│   ├── templates/        # constraint-layer scaffolding copied by /scaffold-constraint-layer
...
- `.github/workflows/ci.yml` — stack-detecting lint + test. A Python repo runs the python job, a Node
  ...
  blocked merge.
- `.github/workflows/review-agent.yml` — runs `/review-diff` against the three docs and comments
...
encode the constraint first via `/encode-constraint`, which walks a first-match-wins rubric and
...
task — the next attempt is blocked by the constraint, not a human. `/agent-playbook-update` is the
...
with `/review-diff` → push → Gate re-checks on PR (CI + review-agent) → if anything slips through,
`/encode-constraint` ratchets the bar up so it can't recur.

**Identifying a constraint-layer gap.** A gap is a class of mistake the layer *should* have caught but
didn't — the rubric never encoded the rule. The tells: `/review-diff` comes back clean yet something
...
`/encode-constraint` to add the missing test/lint/convention, then re-run `/review-diff` so the
next diff of that class is caught mechanically. (See the "Constraint Layer" section in `CLAUDE.md`.)

> **This repo is dual-purpose.** Because it *hosts* pm-os, `/scaffold-constraint-layer` deliberately
...
/scaffold-constraint-layer       # scaffolds docs/, CI, PR template from .claude/templates/constraint-layer/
```
