...
- Use `/gtm-log-lead` to add a **new** prospect.
- Use `/gtm-log-interaction` to append an event to an **existing** lead.
- Use `/ctx-knowledge people` for stakeholders you already work with — not cold prospects.
- Use `/gtm-log-signal` for aggregate market observations across many leads.
...
4. Read `context/ops/leads.md` (create it with the table header if
   ...
   If a match exists, **stop and redirect** to `/gtm-log-interaction`.
...
- If `status = Uncontacted`:
  → "Want me to add a task to `/ops-tasks` to send first outreach?"
...
- If `fit = High` and no persona linked:
  → "Want to link this lead to a persona? Run `/strat-define-persona` if needed."
- If this is the 3rd+ lead from the same source this month:
  → "That channel is producing. Worth logging a signal via `/gtm-log-signal`?"

## Anti-Patterns

- **Existing stakeholder, not a prospect** — redirect to `/ctx-knowledge people`.
- **Aggregate observation** — redirect to `/gtm-log-signal`.
- **A named persona, not a real person** — redirect to `/strat-define-persona`.
