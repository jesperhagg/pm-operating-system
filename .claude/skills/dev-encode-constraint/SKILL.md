...
- The mistake is fuzzy judgment with no clear pattern — use `/dev-agent-playbook-update` to write the paragraph instead.

## Step 1 — Hydrate the Mistake

1. Confirm cwd is a consumer product repo with `docs/agent-playbook.md` and `docs/conventions.md` (the constraint layer is scaffolded). If not, halt: *"Run `/dev-scaffold-constraint-layer` first."*
...
| Is the mistake an **explicit house-style violation** that's hard to express as a lint (e.g., "user-facing errors must return structured objects")? | **Convention entry in `docs/conventions.md`** | Read by `/dev-review-diff` and the review-agent CI workflow as the rubric. |
...
**Strength:** {blocks at CI | blocks at lint | reviewed by /dev-review-diff | absorbed by next session}
...
4. If the agent now ships a clean attempt blocked-then-passing through the constraint, the loop closed. If not, the constraint is too weak — re-run /dev-encode-constraint with the new failure.
```

## Follow-ups

- Mistake was about missing knowledge, not a coding error → `/dev-agent-playbook-update` (this skill's softer cousin)
- Want to verify the constraint actually catches the class on the next diff → `/dev-review-diff`
