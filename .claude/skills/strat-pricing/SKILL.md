...
If you have no persona yet → run `/strat-define-persona` first. Pricing without a persona is guessing what a generic user would pay.
...
If `context/users/personas.md` is empty, halt and suggest `/strat-define-persona` first — pricing without a persona is guessing.
...
**Product:** Hosted `/ops-weekly-review` SaaS for solo multi-product PMs.
...
After delivering the pricing recommendation, append one H3 entry to `context/learnings/pricing.md` (create with `# Learnings — pricing` H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will reconcile.
...
- Validation plan designed → `/disc-design-experiment` to formalize Ship/Kill thresholds for the WTP test.
...
- Pricing changes strategy → `/strat-log-decision` (Type: Go-to-Market) capturing value metric, anchor, and kill criteria.
- WTP test fails → `/ops-sunset-product` (Park with revisit trigger: "pricing works when {condition}").
