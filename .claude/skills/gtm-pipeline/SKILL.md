...
- Use `/gtm-pipeline` for a **CRM-style view**: status buckets, overdue
  follow-ups, stale leads.
- Use `/gtm-log-interaction` to **record** a follow-up once you've done it.
- Use `/ops-weekly-review` for a broader PM review across decisions,
...
If the user passes `/gtm-pipeline {status}` or mentions a single status
...
2. Check that `context/ops/leads.md` exists. If not, surface:
   *"No pipeline yet — run `/gtm-log-lead` to add your first prospect."*
   Do not invent data.
...
- If there are **overdue** follow-ups:
  → "Pick the top one and run `/gtm-log-interaction` once you've sent the
  follow-up, or bump `next_action_date` if you need more time."
- If there are **stale** High-fit leads:
  → "These are leaking. Run `/gtm-log-interaction` with outcome `no-reply`
  and decide: send a break-up email or mark `Lost`."
- If **Uncontacted** > 5:
  → "Backlog's building. Want to add a `/ops-tasks` item to batch 5
  outreach messages?"
- If the pipeline is thin (< 3 active leads across Contacted+):
  → "Pipeline's shallow. Want to run `/gtm-market-scan` or
  `/strat-evaluate-opportunity` on a new channel?"

## Anti-Patterns

- **Don't write anything.** This skill is strictly read-only. If the
  user wants to update a lead, route them to `/gtm-log-interaction`.
