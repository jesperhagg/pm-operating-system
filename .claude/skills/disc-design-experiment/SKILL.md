...
**Falsifiable Prediction:** If we put up a landing page pitching "`/ops-weekly-review` as a service for multi-product PMs" and share it in 3 Slack/Discord communities, signup rate will reach ≥ 8% of unique visitors within 14 days.
...
**Decision Rule:**
- Ship → `/strat-write-prd` for the hosted product, log Decision (Go-to-Market): *"Build weekly-review SaaS, wedge = solo-PM Slack communities."*
- Kill → `/strat-log-decision` (Kill): *"No demand signal for hosted weekly-review. Park for 6 months."* Log Signal (Internal Learning) with reply-rate data.
...
After delivering the experiment design, append one H3 entry to `context/learnings/design-experiment.md` (create with `# Learnings — design-experiment` H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will reconcile.
...
- Experiment designed → confirm the user commits the Decision Rule before launch (via `/strat-log-decision` with Type: Go-to-Market, Outcome: Pending).
- Result came in (Ship) → `/strat-write-prd` for the shippable version.
- Result came in (Kill) → `/strat-log-decision` (Kill/Park) + `/gtm-log-signal` (Internal Learning with data).
...
- **Building a full product as 'the experiment'** — If the MVT is 4 weeks of build, it's not an experiment, it's a bet. Shrink it or admit it's a bet and use `/strat-write-prd`.
