...
or when a `/ops-weekly-review` keeps surfacing "what are we actually
optimizing for?"
...
> `/strat-evaluate-opportunity` (or `/ops-sunset-product`) before adding more
...
`# Strategy` H1 (defensive; `/ctx-context-init` scaffolds it).
...
`/strat-evaluate-opportunity` before adding scope, headcount, or spend.
...
8 consecutive weeks, run `/strat-evaluate-opportunity` before adding scope.
...
`session:pending` — `/ctx-context-sync` will reconcile.
...
- NSM defined → `/strat-log-decision` (Type: Strategy) to formally commit the
  pick as a decision tied to current strategy.
- An input metric is unmeasured today → `/disc-design-experiment` for a
  lightweight instrumentation test.
- Weekly cadence needs reorienting → next `/ops-weekly-review` will anchor
  Focus Score against the NSM trajectory.
- Kill-criterion is going to trigger soon (already flat) → run
  `/strat-evaluate-opportunity` *now*, don't wait for the timer.
