...
If `context/users/personas.md` is empty or `context/product/decisions.md` has no Active decisions, halt and say so — do not write a PRD without grounded context. Suggest running `/strat-define-persona` or capturing prior decisions first.
...
**Example:**
> We believe solo PMs will run `/ops-weekly-review` every Monday because they already have the habit of a Monday review but lack structured tooling.
>
...
After delivering the PRD, append one H3 entry to `context/learnings/write-prd.md` (create with `# Learnings — write-prd` H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will reconcile.
...
**Problem Statement (for `/ops-weekly-review` SaaS):**
...
**Hypothesis:**
> We believe solo PMs with 2+ products will run a hosted `/ops-weekly-review` every Monday because they already block Monday time for review but hate the manual aggregation.
>
...
- Problem section cited no Signals → suggest `/gtm-log-signal` to capture user feedback before building, OR running user interviews.
- Risky assumption is buildable-to-test → suggest `/disc-design-experiment` to frame the validation test.
- PRD is scoped → suggest `/disc-break-down` to decompose into kanban-ready tasks.
- PRD introduces AI or expensive infra → consult **systems-architect** for cost model.
- PRD represents a new product bet → suggest `/strat-evaluate-opportunity` before committing.
