...
If `context/product/decisions.md` has no Active entries AND `context/market/landscape.md` has no scans, warn the user — scoring without any prior context is guesswork. Suggest running `/gtm-market-scan` first, or proceed with assumptions flagged explicitly.
...
- Add a task via `/ops-tasks add` for the first validation step with `priority:now`.
- Log a Decision via `/strat-log-decision` (Type: Kill/Park/Scope/Positioning) if this supersedes an active bet.
```
...
### Strategy A — Hosted /ops-weekly-review SaaS
- **Shape:** $19/mo hosted version of the existing `/ops-weekly-review` skill. Login + Notion OAuth. Weekly digest emailed.
...
After delivering the evaluation, append one H3 entry to `context/learnings/evaluate-opportunity.md` (create with `# Learnings — evaluate-opportunity` H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will reconcile.
...
- **Pursue** → suggest `/disc-design-experiment` to frame the first validation test for the chosen strategy.
- **Pursue narrow** + low Feasibility → suggest consulting the **systems-architect** agent on the tech cost ceiling.
- **Park** → `/strat-log-decision` with Type: Park, noting the trigger to revisit.
- **Kill** → `/strat-log-decision` with Type: Kill, linking the evaluation as context.
