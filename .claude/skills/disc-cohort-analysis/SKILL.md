...
keep v2 as default and run `/strat-log-decision` (Type: Scope) to make it
...
- Divergence found, cause unknown → `/disc-design-experiment` to test the
  hypothesis behind it.
- Behavior shift is the headline → `/gtm-log-signal` (Type: Internal
  Learning) with the cohort data inline.
- Divergence implies a decision → `/strat-log-decision` (Type: Scope or
