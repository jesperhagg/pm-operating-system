---
description: Capture non-obvious knowledge the coding agent uncovered during a session into docs/agent-playbook.md so the next session inherits it. Closes the "recursing six levels deep for credentials" trap. Softer cousin of /dev-encode-constraint — used when the gap is missing knowledge, not a coding mistake.
---
...
**When NOT to run:**
- The gap is a class of coding mistake — use `/dev-encode-constraint` instead (stronger artifact).
- The knowledge is one-off and won't recur — don't pollute the playbook.

## Step 1 — Hydrate

1. Confirm cwd is a consumer product repo with `docs/agent-playbook.md`. If not, halt: *"Run `/dev-scaffold-constraint-layer` first."*
...
- If the same knowledge gap caused a coding mistake (not just lost time) → also run `/dev-encode-constraint` to add a stronger artifact (test or lint).
...
- Multiple onboarding gaps caught in one session → `/dev-encode-constraint` for the architectural ones; rest into the playbook.
...
- **Don't add a knowledge entry for a class of coding mistake.** That's `/dev-encode-constraint`'s job — a lint or a test, not a paragraph.
