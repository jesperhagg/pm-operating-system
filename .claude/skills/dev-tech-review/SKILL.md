...
```
/dev-tech-review                                    # whole repo, all lenses
/dev-tech-review packages/api                       # path scope, all lenses
/dev-tech-review packages/api security,performance  # path + lens filter
/dev-tech-review . simplify                         # whole repo, single lens
```
...
After the audit (and any Fix-mode application) completes, append one H3 entry to `context/learnings/tech-review.md` (create with `# Learnings — tech-review` H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will reconcile.
...
- Architecture-level findings (drift from adapter pattern, missing layer,
  recurring lens hits across runs) → run `/strat-log-decision` to record the
