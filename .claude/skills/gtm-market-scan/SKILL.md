---
name: gtm-market-scan
description: Scan the competitive landscape for the product, discovering active competitors, recent product launches, funding moves, and customer sentiment. Dual-writes findings to context/ — durable synthesis to context/market/landscape.md, time-stamped observations to context/market/signals.md. Use when the user invokes /gtm-market-scan.
---
...
2. If a market category argument is provided (e.g., `/gtm-market-scan
...
following the format in `/gtm-log-signal`. Add `session:pending` annotation.
...
After the dual-write completes, append one H3 entry to `context/learnings/market-scan.md` (create with `# Learnings — market-scan` H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will reconcile.
...
- If any Signal had `action_required:true`:
  → "Run `/ops-weekly-review` to see all Action Required signals, or
  `/strat-log-decision` to commit to a response?"
- If Strategic Implications surfaced a new opportunity:
  → "Run `/strat-evaluate-opportunity` on the {opportunity} thread?"
- If the scan contradicted a prior positioning decision:
  → "This scan contradicts the {date} positioning decision. Run
  `/strat-log-decision` to log a superseding decision?"
- If the scan exposed a capability gap:
  → "Run `/ctx-knowledge research \"{topic}\"` to pull related research?"
