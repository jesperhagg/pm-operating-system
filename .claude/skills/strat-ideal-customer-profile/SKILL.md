...
- Before `/strat-pricing` — pricing without an ICP overweights generic
...
If the user wants a persona instead, redirect to `/strat-define-persona`.
...
exist, create it with `# ICP` H1 header (defensive — `/ctx-context-init`
...
- ICP defined → `/gtm-log-lead` to apply the fit-score rubric to current
  prospects in `context/ops/leads.md`.
- Buying committee differs from expected WTP → `/strat-pricing` to revise
  tiers against the ICP's actual price ceiling.
- ICP narrowed the persona → `/strat-define-persona` to refine the primary
