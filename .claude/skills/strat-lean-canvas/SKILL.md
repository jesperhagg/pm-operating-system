...
`/strat-define-persona` or `/strat-ideal-customer-profile` first — the Customer Segment
...
**Rules:**
- Value metric + anchor price (run `/strat-pricing` for the full structure if
...
- One outcome metric (retention, revenue, activation rate). Pick the one
  closest to actual customer value — see `/strat-north-star-metric` if you need
...
H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will
reconcile.
...
- Riskiest box identified → `/disc-design-experiment` to frame the falsification
  test for that single box.
- Revenue Streams thin or `assumption: untested` → `/strat-pricing` to set value
  metric, anchor, and WTP validation.
- Customer Segment vague or no ICP on file → `/strat-ideal-customer-profile`.
- Problem box has < 2 evidence citations → `/gtm-log-signal` to capture user
  interviews before committing.
- Multiple boxes untested → `/strat-evaluate-opportunity` to score whether the
