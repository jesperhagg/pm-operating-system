...
This skill defines **customer personas** — who is being sold to / built for. It does NOT define stakeholders (colleagues, investors, partners). For stakeholder notes, use `/ctx-knowledge people`.
...
After delivering the persona, append one H3 entry to `context/learnings/define-persona.md` (create with `# Learnings — define-persona` H1 if missing). Newest first. Mark `session:pending` — `/ctx-context-sync` will reconcile.
...
- Evidence is Thin → `/gtm-log-signal` after each user interview; run this skill again in 30 days.
- Persona is sharp + product needs specing → `/strat-write-prd` referencing this persona.
- Persona reveals a distribution trigger → consult **growth-engineer**.
- Persona conflicts with existing positioning → `/strat-log-decision` (Type: Positioning).
- Anti-persona is actually the bigger market → `/strat-evaluate-opportunity`.
