---
description: Fetch, store, and review structured knowledge in context/. Manages people (stakeholders), research (domain insights), and strategy/reference. Market landscape is read-only here, written by /gtm-market-scan.
---
...
- **market-landscape** (`context/market/landscape.md`) — living competitive
  document. Written exclusively by `/gtm-market-scan` as append-only dated
  sections. Read by this skill but not edited directly.

**Scope boundary:** Knowledge stores **durable, synthesized understanding**.
Time-stamped observations belong in Signals — use `/gtm-log-signal`. Customer
personas use `/strat-define-persona`. See the Context Routing Rubric in
...
**Triggers:** `/ctx-knowledge people "name"`, `/ctx-knowledge research "topic"`,
`/ctx-knowledge strategy`, or just `/ctx-knowledge "search term"`
...
**Triggers:** `/ctx-knowledge add`, `/ctx-knowledge store`

**Steps:**

1. Ask the user for:
   - **Category:** people, research, or strategy (Market Landscape is
     reserved for `/gtm-market-scan`).
   - **Title:** name or topic.
...
redirect: *"That sounds like a Signal — run `/gtm-log-signal` instead."*
If it's a customer persona, redirect to `/strat-define-persona`.

### 3. Review

**Triggers:** `/ctx-knowledge review`
...
- Re-scan: market landscape past 30 days — suggest /gtm-market-scan
```
...
Market Landscape entries are maintained by `/gtm-market-scan` — this skill
...
→ "Want to check how this connects to your current backlog? Run `/ctx-fetch-context`."

After fetching a Market Landscape entry older than 30 days:
→ "This scan is {N} days old. Want to run `/gtm-market-scan` to refresh it?"
